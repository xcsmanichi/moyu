...
const { search } = require("./routes/api.js");
...
router.get('/jadwaltv', async (req, res, next) => {
	var jadwal = req.query.jadwal
		if (!jadwal) return res.json({status: false, creator, code: 404, message: 'masukan parameter jadwal, jadwal yang tersedia : rcti, nettv, antv, gtv, indosiar, inewstv, kompastv, metrotv, mnctv, rtv, sctv, trans7, transtv, tvone, tvri'})
		if (jadwal == 'rcti') {
            jadwaltv(`${jadwal}`)
			.then(result => {
				res.json({
