...
async function ytmp3(link){
	return new Promise(async (resolve, reject) => {
		const ytIdRegex = /(?:http(?:s|):\/\/|)(?:(?:www\.|)youtube(?:\-nocookie|)\.com\/(?:watch\?.*(?:|\&)v=|embed\/|v\/)|youtu\.be\/)([-_0-9A-Za-z]{11})/
			...
			axios('https://www.y2mate.com/mates/en402/analyze/ajax', {
				method: 'POST',
				data: new URLSearchParams(Object.entries(config)),
				headers: {
			...
			.then(async (res) => {
				...
				await axios({
                    ...
                    const $ = cheerio.load(body.data.result)
...
async function ytmp4(link){
	return new Promise(async (resolve, reject) => {
		const ytIdRegex = /(?:http(?:s|):\/\/|)(?:(?:www\.|)youtube(?:\-nocookie|)\.com\/(?:watch\?.*(?:|\&)v=|embed\/|v\/)|youtu\.be\/)([-_0-9A-Za-z]{11})/
			...
			.then(async (res) => {
				var $ = cheerio.load(res.data.result)
				...
				await axios({
                    ...
                    var $ = cheerio.load(body.data.result)
                    var urlDown = $('div[class="form-group has-success has-feedback"]').find('a').attr("href");
                    resolve({
                        title: `${title}`, 
                        size: `${size}`, 
