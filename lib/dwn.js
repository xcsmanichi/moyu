...
async function ytmp3(link){
	return new Promise(async (resolve, reject) => {
		const ytIdRegex = /(?:http(?:s|):\/\/|)(?:(?:www\.|)youtube(?:\-nocookie|)\.com\/(?:watch\?.*(?:|\&)v=|embed\/|v\/)|youtu\.be\/)([-_0-9A-Za-z]{11})/
			...
			axios('https://www.y2mate.com/mates/id4/analyze/ajax', {
				method: 'POST',
				data: new URLSearchParams(Object.entries(config)),
				headers: {
					"sec-ch-ua": '"Google Chrome";v="105", "Not)A;Brand";v="8", "Chromium";v="105"',
					"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36",
					"Cookie": '_gid=GA1.2.2114706683.1664088530; _gat_gtag_UA_84863187_21=1; _ga_K8CD7CY0TZ=GS1.1.1664088530.1.1.1664088536.0.0.0; _ga=GA1.2.1592419843.1664088530'
				}
			})
			.then(({data})=> {
				...
				axios({
                    method: 'post',
                    url: 'https://www.y2mate.com/mates/id4/convert',
                    headers: {
                      accept: "*/*",
	...
	return new Promise(async (resolve, reject) => {
		const ytIdRegex = /(?:http(?:s|):\/\/|)(?:(?:www\.|)youtube(?:\-nocookie|)\.com\/(?:watch\?.*(?:|\&)v=|embed\/|v\/)|youtu\.be\/)([-_0-9A-Za-z]{11})/
			...
			.then(({data})=> {
				...
				var imageSrc = $('div[class="thumbnail cover"]').find('a > img').attr('src'),
                title = $('div[class="caption text-left"]').find('b').text(),
...
async function nhen(code){
    return new Promise((resolve, reject) => {
        try{
        axios.get('https://nhentai.net/g/' + code, {
            headers: {
                "cookie": "cf_clearance=bgo5y7rUzlAwR_FcgxRuDMVQOEYBgd65159KbPBKnpU-1665903154-0-150; csrftoken=ol5BW0PkEairXxaTVMNoAOchQWkJS0qZKBVdFXdtoOl5nnUAG4xRHZombppXYsKa",
            ...
            $('div.thumbs > div.thumb-container').each(function (a,b) {
                const link = $(b).find('a > img').attr('data-src')
                hasil.push(link)
            })
            resolve(hasil)
        })
